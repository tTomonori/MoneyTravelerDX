...
public class GameXAndSlopeCamera : GameCamera {
    //撮影対象の場所へ移動する
    public override void moveToTarget() {
        ...
        if (tDistance < mMoveSpeed * Time.deltaTime) {
            this.worldPosition = new Vector3(mTarget.transform.position.x, mTarget.transform.position.z, mTarget.transform.position.z);
            return;
        }
    ...
    public override void shoot(MonoBehaviour aTarget) {
        this.worldPosition = new Vector3(aTarget.transform.position.x, aTarget.transform.position.z, aTarget.transform.position.z);
    }
    public override void move(Vector2 aVec) {
        this.position += new Vector3(aVec.x, aVec.y, aVec.y);
